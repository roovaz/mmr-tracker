...
async function loadJobs() {
    ...
    const url = status
        ? `${API_BASE}/api/admin/jobs?status=${status}&limit=100`
        : `${API_BASE}/api/admin/jobs?limit=100`;
...
async function loadUsers() {
    try {
        const response = await authFetch(`${API_BASE}/api/admin/users?limit=100`);
...
async function showJobDetail(jobId) {
    try {
        const response = await authFetch(`${API_BASE}/api/admin/jobs/${jobId}`);
...
async function showSnapshotDetail(snapshotId) {
    try {
        const response = await fetch(`${API_BASE}/api/profile/${snapshotId.split('/').pop()}`);

        // Try to get from leaderboard API if direct fetch fails
        let snap;
        if (!response.ok) {
            // Fetch from admin users endpoint and find the snapshot
            const usersResponse = await authFetch(`${API_BASE}/api/admin/users`);
            const users = await usersResponse.json();

            for (const user of users) {
                const found = user.snapshots.find(s => s.id === snapshotId);
                if (found) {
                    snap = { ...found, username: user.username };
                    break;
                }
            }
        } else {
        ...
        modalBody.innerHTML = `
            ...
            <div class="detail-section">
                <h3>Full URL</h3>
                <div class="detail-value">
                    <a href="${snap.full_url || snap.profile_url}" target="_blank" style="color: var(--primary);">${snap.full_url || snap.profile_url}</a>
                </div>
            </div>
